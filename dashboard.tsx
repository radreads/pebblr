...
import { Bell, Check, ChevronDown, Gift, Plus, Search, Settings } from "lucide-react"
      ...
      <main className="mx-auto max-w-2xl px-4 py-8">
        <div className="space-y-6">
          ...
          <Tabs defaultValue="all" className="w-full">
            <TabsList className="w-full grid grid-cols-4 bg-gray-100 p-1">
              <TabsTrigger value="all" className="data-[state=active]:bg-cyan-700 data-[state=active]:text-white">
                All
              </TabsTrigger>
              <TabsTrigger value="monthly" className="data-[state=active]:bg-cyan-700 data-[state=active]:text-white">
                Monthly
              </TabsTrigger>
              <TabsTrigger value="quarterly" className="data-[state=active]:bg-cyan-700 data-[state=active]:text-white">
                Quarterly
              </TabsTrigger>
              <TabsTrigger value="overdue" className="data-[state=active]:bg-cyan-700 data-[state=active]:text-white">
                Overdue
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="all" className="mt-6">
              <div className="space-y-3">
                {contactsList.map((contact) => (
                  <ContactCard key={contact.id} contact={contact} onMarkDone={() => markAsDone(contact.id)} />
                ))}
              </div>
            </TabsContent>
            ...
            <TabsContent value="overdue" className="mt-6">
              <div className="space-y-3">
                {contactsList
                  .filter((contact) => contact.daysOverdue > 7)
                  .map((contact) => (
                    <ContactCard key={contact.id} contact={contact} onMarkDone={() => markAsDone(contact.id)} />
                  ))}
              </div>
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-cyan-950">Add New Contact</DialogTitle>
              <DialogDescription className="text-cyan-700">
                Add someone you want to stay in touch with regularly.
              </DialogDescription>
            </DialogHeader>
...
function ContactCard({ contact, onMarkDone }: ContactCardProps) {
